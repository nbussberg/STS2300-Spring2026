...
2. The code below will give us the *first* table on the Wikipedia page. Is this what we want? If not, how could we change the code to get the large table of winners and nominees? Change the code below and answer below what you did.
...
3. Look up the help file for the `rename()` function from the `dplyr` package. Use the function to rename the `Author(s)` column to just Author so that we get rid of the parentheses in the name. (Note: When referencing a column with special characters or spaces in it, you need to put tick marks around the name. This is the same symbol we used for in-line R code in Activity 02, and it is found below the Esc key.)
